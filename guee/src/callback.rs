...
impl<P> Callback<P> {
    ...
    pub fn call(self, input: &mut dyn Any, payload: P) {
        (self.f)(input, payload);
    }
}
...
impl AccessorRegistry {
    ...
    pub fn invoke_callback(&self, state: &mut dyn Any, cd: CallbackDispatch) {
...
/// A dispatched callback is a type-erased callback (no generic P) plus its
/// type-erased payload and an invoker function that can be used to run the
/// actual callback code.
///
/// Callback dispatches are generated by widgets code, typically during
/// on_event, to enqueue some things to be called later.
pub struct CallbackDispatch {
    // The input type of the callback
    input_type: TypeId,
    // The type-erased callback
...
impl CallbackDispatch {
    pub fn new<P: 'static>(c: Callback<P>, payload: P) -> Self {
        let closure = |cb: Box<dyn Any>, input: &mut dyn Any, p: Box<dyn Any>| {
            let cb: Callback<P> = *cb.downcast().expect("Downcast failed");
...
mod tests {
    ...
    use crate::callback::{Callback, CallbackDispatch};
    ...
    fn test_accessor_regitry() {
        ...
        *y = 9.99;
        ...
        let cd = CallbackDispatch::new(cb, ());
